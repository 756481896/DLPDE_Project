...
class Logger():
    ...
    def summarize(self, step, summerizer="train", scope="summary", summaries_dict=None):
        """
        ...
        """
        summary_writer = self.train_summary_writer if summerizer == "train" else self.test_summary_writer
        with variable_scope.variable_scope(scope) as summary_scope:
                ...
                for tag, value in summaries_dict.items():
                    with variable_scope.variable_scope("test"):
                        if tag not in self.summary_ops:
                            ...
                            self.summary_placeholders[tag] = tf.placeholder('float64',name=tag)
